...
## Conclusion and Discussion
    During the application development process, I realized that my team has to improve time management and use GitHub as well as teamwork. Although the team's application contains several bugs and there is not a single feature that operates well. We have an issue between the database and also the Netbeans version when I fail to insert data into the database and my Netbeans frequently fail to connect to the database. For all of the above, we need to split tasks among members and code consistency across functions so that we don't take more time fixing issues. 
